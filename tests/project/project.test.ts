...
describe("Aira project initialization", () => {
  ...
  test("is idempotent and does not overwrite existing project files", async () => {
    ...
    const workflowPath = path.join(first.paths.workflowsDir, "keep.yaml");
    const commandPath = path.join(first.paths.commandsDir, "keep.md");
    ...
    await Promise.all([
      ...
      writeFile(workflowPath, "keep workflow", "utf8"),
      writeFile(commandPath, "keep command", "utf8"),
    ...
    expect(await readFile(workflowPath, "utf8")).toBe("keep workflow");
    expect(await readFile(commandPath, "utf8")).toBe("keep command");
